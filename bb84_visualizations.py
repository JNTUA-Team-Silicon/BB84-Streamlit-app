...
# ============================================================
def plot_pdf_style_timeline(timeline_df, title="BB84 Timeline", max_bits=50, color_scheme='blue'):
    ...
    Args:
        ...
        color_scheme: 'blue' for No Eve, 'red' for With Eve
    ...
    alice_primary_light = '#ADD8E6'  # light cyan blue
    alice_primary_dark = '#00008B'   # dark blue
    alice_edge_color = '#0000CD'     # medium blue edge
    
    # Other colors vary by scenario (blue for No Eve, red for With Eve)
    if color_scheme == 'red':  # With Eve - Red for base matching and transmission results
        match_color = '#E74C3C'    # match red
        error_color = 'red'         # error red
    else:  # Blue (No Eve)
        match_color = '#5DADE2'    # match blue
        error_color = '#2E86AB'    # error blue

    # Plot 1: Alice's bits with visibility (ALWAYS BLUE regardless of scenario)
    colors1 = []
    edge_colors1 = []
    heights1 = []
    ...
    for bit in display_df['AliceBit']:
        if bit == 0:
            colors1.append(alice_primary_light)
            edge_colors1.append(alice_edge_color)
            heights1.append(0.2)
        else:
            colors1.append(alice_primary_dark)
            edge_colors1.append(alice_edge_color)
            heights1.append(1.0)
    ...
    # Plot 2: Base matching - Green for match, Red for mismatch
    colors2 = ['green' if m else 'red' for m in display_df['BaseMatch']]
    ...
    # Plot 3: Error status - Green for correct, Red for error, Gray for not used
    error_heights = []
    colors3 = []
...
def create_pdf_report_with_graphs(
    ...
    with PdfPages(buf) as pdf:
        ...
        lines.append("-" * 60)
        lines.append(f"Without Eve: QBER = {qber_no:.3f}, Key = {key_no} bits")
        lines.append(f"With Eve:    QBER = {qber_e:.3f}, Key = {key_e} bits")
        lines.append("")
        lines.append("Bit Distribution Summary:")
        lines.append(f"No Eve scenario - 0-bits: {num_bits//2}, 1-bits: {num_bits//2}")
        lines.append(f"With Eve scenario - 0-bits: {num_bits//2}, 1-bits: {num_bits//2}")
        ...
        colors_eve = ['#E74C3C', '#C0392B', '#A93226']     # Red shades for with eve

        # No Eve chart
        ax1.bar(["Transmitted", "Sifted", "Final Key"], [num_bits, sift_no, key_no], color=colors_no_eve)
        ax1.set_title(f"No Eve (QBER: {qber_no:.3f})", fontsize=12, fontweight="bold")
        ax1.set_ylabel("Number of Bits", fontsize=11)
        ax1.grid(axis='y', alpha=0.3, linestyle='--')

        # With Eve chart
        ax2.bar(["Transmitted", "Sifted", "Final Key"], [num_bits, sift_e, key_e], color=colors_eve)
        ax2.set_title(f"With Eve (QBER: {qber_e:.3f})", fontsize=12, fontweight="bold")
        ...
        comparison_lines = []
        comparison_lines.append("DETAILED COMPARISON ANALYSIS")
        comparison_lines.append("=" * 60)
        comparison_lines.append("")
        comparison_lines.append("WITHOUT EVE SCENARIO")
        comparison_lines.append("-" * 60)
        comparison_lines.append(f"QBER (Quantum Bit Error Rate): {qber_no:.3f}")
        comparison_lines.append(f"Final Key Length: {key_no} bits")
        comparison_lines.append(f"Sifted Bits: {sift_no} bits")
        comparison_lines.append(f"Transmitted Bits: {num_bits} bits")
        comparison_lines.append("")
        comparison_lines.append("WITH EVE SCENARIO")
        comparison_lines.append("-" * 60)
        comparison_lines.append(f"QBER (Quantum Bit Error Rate): {qber_e:.3f}")
        comparison_lines.append(f"Final Key Length: {key_e} bits")
        comparison_lines.append(f"Sifted Bits: {sift_e} bits")
        comparison_lines.append(f"Transmitted Bits: {num_bits} bits")
        comparison_lines.append("")
        comparison_lines.append("KEY DIFFERENCES")
        comparison_lines.append("-" * 60)
        comparison_lines.append(f"QBER Increase: {qber_e - qber_no:.3f}")
        comparison_lines.append(f"Key Loss (bits): {key_no - key_e} bits")
        comparison_lines.append(f"Sifting Reduction: {sift_no - sift_e} bits")
        comparison_lines.append("")
        comparison_lines.append("SECURITY ASSESSMENT")
        comparison_lines.append("-" * 60)
        if qber_e > threshold:
            comparison_lines.append(f"✅ Eavesdropping DETECTED")
            comparison_lines.append(f"QBER ({qber_e:.3f}) exceeds threshold ({threshold:.3f})")
            comparison_lines.append("Recommendation: ABORT key exchange")
        else:
            comparison_lines.append(f"✅ Channel appears SECURE")
            comparison_lines.append(f"QBER below threshold ({threshold:.3f})")
            comparison_lines.append("Recommendation: Accept shared key")

        fig5.text(0.06, 0.95, "\n".join(comparison_lines), va="top", fontsize=10, family="monospace")
